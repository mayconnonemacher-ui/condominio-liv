...
for c in CATS:
    ...
    cost_rows += f'<tr><td>{e(c).replace("/", "/&#8203;")}</td><td><span class="prog">{prog}</span> {got}/3</td><td class="n">{brl(min(vals)) if vals else "—"}</td><td class="n c-md">{brl(sum(vals)/len(vals)) if vals else "—"}</td><td class="n c-md">{brl(max(vals)) if vals else "—"}</td><td class="c-md">{e(ctr[0]["empresa"]) + " · " + brl(ctr[0].get("valorMensal")) if ctr else "<span class=muted>em cotação</span>"}</td></tr>'
...
<p class="muted" style="font-size:12px;margin:0 0 8px">Os fixos saem todo mês; os periódicos saem só quando vencem e são pagos com o fundo de provisões, alimentado por {brl(prov_m)} por mês. Fundo negativo nos primeiros meses indica despesa periódica que vence antes de a provisão estar formada — é o que o fundo inicial de instalação cobre. A coluna Entradas inclui os boletos e, quando houver, as parcelas do fundo de instalação recebidas no mês. Fixos e periódicos por mês de competência (o pagamento cai no dia {dpag} do mês seguinte); caixa livre pelo calendário real, já sem o fundo de reserva.</p>
...
.intro{{color:var(--ink-2);margin:0 0 20px}}
.kpis{{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:10px;margin-bottom:22px}}
.kpi{{background:var(--surface);border:1px solid var(--line);border-radius:10px;padding:12px 14px}}.kpi .lbl{{font-size:12px;color:var(--ink-2)}}.kpi .val{{font:600 26px/1.1 "Sora",sans-serif;margin-top:4px;font-variant-numeric:tabular-nums}}.kpi .val{{white-space:nowrap;font-size:clamp(19px,2vw,26px)}}.kpi .val small{{font-size:14px;color:var(--ink-3)}}.kpi .sub{{font:400 11px "IBM Plex Mono",monospace;color:var(--ink-3);margin-top:4px}}
...
.two{{display:grid;grid-template-columns:2.6fr 1fr;gap:16px}}.two td,.two th{{white-space:nowrap;padding-left:7px;padding-right:7px}}.two td:first-child,.two .panel td:last-child{{white-space:normal}}@media(max-width:1000px){{.two{{grid-template-columns:1fr}}}}
...
tr.tot td{{font-weight:600;background:var(--surface-2)}}tr.grp td{{background:var(--surface-2);font-weight:600;font-size:12.5px;padding-top:10px}}tr.grp td small{{display:block;font-weight:400;color:var(--ink-3);font-size:11.5px}}tr.done td{{background:color-mix(in srgb,var(--good-soft) 55%,transparent)}}tr.out td{{opacity:.5}}
...
@media(max-width:600px){{.two .panel td,.two .panel th{{padding-left:4px;padding-right:4px}}#orcamento .pill+small{{display:none}}#orcamento th,#orcamento td{{padding:6px 4px;font-size:12px}}#orcamento .pill{{font-size:10px;padding:2px 6px}}#orcamento td.n small{{display:none}}}}
.intro,.dem .dd,.dem .dl,#orcamento p,#orcamento details p,.panel>p.muted,footer,tr.grp td small{{text-align:justify;text-align-last:left;hyphens:auto;-webkit-hyphens:auto}}.kpis{{align-items:stretch}}.kpi{{display:flex;flex-direction:column}}.kpi .sub{{margin-top:auto;padding-top:4px}}.dem .dm{{justify-content:flex-start}}
